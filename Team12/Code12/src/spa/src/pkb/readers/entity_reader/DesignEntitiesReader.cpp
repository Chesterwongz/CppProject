...
std::string DesignEntitiesReader::getEntityBy(int statementNumber, StmtType stmtType, SStoreType& sStore) {
    std::vector<std::string> stmts = getAllStmtsOf(stmtType);
    std::vector<std::string> results = sStore.getAllDirectSuccessorsOf(statementNumber);

    if (results.empty() ||
        std::find(stmts.begin(), stmts.end(),
            std::to_string(statementNumber)) == stmts.end()) {
        return "";
    }
    return results.front();
}
