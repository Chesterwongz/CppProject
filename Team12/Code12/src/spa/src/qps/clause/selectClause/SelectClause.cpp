...
IntermediateTable SelectClause::evaluate(
        Context &context,
        PKBReader &pkb) {
...
IntermediateTable SelectClause::getAllPossibleValues(
        Context &context,
        PKBReader &pkb,
        unique_ptr<IArgument> &synonymArg) {
    string synonymValue = synonymArg->getValue();
    Entity entity = context.getTokenEntity(synonymValue);
    set<string> results;

    bool isStmtEntity = STATEMENT_ENTITIES.find(entity) != STATEMENT_ENTITIES.end();
    ...
    return IntermediateTableFactory::buildIntermediateTable(synonymValue, results);
}
