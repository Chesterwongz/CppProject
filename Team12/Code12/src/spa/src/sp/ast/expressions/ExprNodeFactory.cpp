...
std::unique_ptr<TNode> ExprNodeFactory::makeNode(const std::string &kOp,
                                                 std::unique_ptr<TNode> leftExpr,
                                                 std::unique_ptr<TNode> rightExpr) {
    // Arithmetic op
    if (kOp == op::kPlus)
        return std::make_unique<PlusNode>(std::move(leftExpr), std::move(rightExpr));
    if (kOp == op::kMinus)
        return std::make_unique<MinusNode>(std::move(leftExpr), std::move(rightExpr));
    if (kOp == op::kTimes)
        return std::make_unique<TimesNode>(std::move(leftExpr), std::move(rightExpr));
    if (kOp == op::kDiv)
