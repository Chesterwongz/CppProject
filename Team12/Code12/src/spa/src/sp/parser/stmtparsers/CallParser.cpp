...
std::optional<std::unique_ptr<TNode>> CallParser::parse() {
    std::optional<std::string> procNameOpt = context->tryEatExpected(SpTokenType::NAME);
    ...
    unique_ptr<TNode> callNode = std::make_unique<CallNode>(procNameOpt.value(), context->getLineNum());
    return callNode;
}
