...
bool ParserContext::isExpected(SpTokenType expectedType, const std::string &expectedValue) {
    return isExpected(expectedType) && tokenStream->peek()->getValue() == expectedValue;
}

std::optional<std::string> ParserContext::tryEatExpected(SpTokenType expectedType) {
    if (!isExpected(expectedType)) return std::nullopt;
    return tokenStream->eat()->getValue();
}

std::optional<std::string> ParserContext::tryEatExpected(SpTokenType expectedType, const std::string &expectedValue) {
...
std::string ParserContext::forceEatExpected(SpTokenType expectedType, const std::string &expectedValue) {
