...
InputStream::InputStream(std::string fileContents)
    : fileContents(std::move(fileContents)), cursor(0) {}

bool InputStream::isEnd() {
  return this->cursor >= this->fileContents.length();
}

char InputStream::peek() { return fileContents.at(cursor); }

std::string InputStream::peekWhile(
...
std::string InputStream::readWhile(
    const std::function<bool(const char)> &predicate) {
  std::string str;
