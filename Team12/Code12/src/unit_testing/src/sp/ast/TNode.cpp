...
std::unique_ptr<RelNode> makeRelNode1() {
    return std::make_unique<LteNode>(
        std::make_unique<VarNode>("x"),
        std::make_unique<ConstNode>("0")
    );
}
...
std::unique_ptr<RelNode> makeRelNode2() {
    return std::make_unique<GteNode>(
        std::make_unique<VarNode>("x"),
        std::make_unique<ConstNode>("0")
    );
}
...
TEST_CASE("Test TNode equality - NotNode") {
    std::unique_ptr<TNode> node1 = std::make_unique<NotNode>(std::move(makeRelNode1()));
    std::unique_ptr<TNode> node2 = std::make_unique<NotNode>(std::move(makeRelNode1()));
    REQUIRE(node1->isEqual(*node2));
    REQUIRE(*node1 == *node2);
}
...
TEST_CASE("Test TNode equality - WhileNode with same children different line number") {
    std::unique_ptr<TNode> node1 = std::make_unique<WhileNode>(1);
