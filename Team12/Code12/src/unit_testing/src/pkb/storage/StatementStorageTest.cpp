...
TEST_CASE("StatementStorage Tests") {
    ...
    SECTION("setStatement and getStatementNumbersFromStatementType") {
        REQUIRE(storage.getStatementNumbersFromStatementType(StmtType::IF) == std::set<int>{1, 2});
        REQUIRE(storage.getStatementNumbersFromStatementType(StmtType::WHILE) == std::set<int>{3});
        REQUIRE(storage.getStatementNumbersFromStatementType(StmtType::ASSIGN) == std::set<int>{4});
        REQUIRE(storage.getStatementNumbersFromStatementType(StmtType::CALL) == std::set<int>{});
    }
    ...
    SECTION("getStatementTypeFromStatementNumber") {
        ...
        REQUIRE(storage.getStatementTypeFromStatementNumber(7) == StmtType::INVALID);
    }
